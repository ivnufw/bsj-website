...
// =======================================================
$staff_group = 'วิทยาศาสตร์'; // ชื่อกลุ่มใน DB 
$staff_group_title = 'กลุ่มสาระการเรียนรู้วิทยาศาสตร์และเทคโนโลยี';
$staff_color_primary = '#00008B'; // Dark Blue
$staff_color_secondary = '#F0F8FF'; // AliceBlue (พื้นหลัง)
$staff_color_accent = '#ADD8E6'; // Light Blue Accent
...
<head>
    ...
    <style>
        ...
        .thai-staff-section { background-color: <?php echo $staff_color_secondary; ?>; padding: 40px 20px; border-radius: 10px; box-shadow: 0 0 15px rgba(0,0,0,0.05); }
        .thai-staff-header { color: <?php echo $staff_color_primary; ?>; font-weight: 700; border-bottom: 5px double <?php echo $staff_color_accent; ?>; border-top: 1px solid <?php echo $staff_color_primary; ?>; padding-bottom: 10px; margin-bottom: 40px; text-align: center; }
        .staff-card { border: 2px solid <?php echo $staff_color_accent; ?>; border-radius: 15px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1); transition: transform 0.3s, box-shadow 0.3s; overflow: hidden; background-color: white; text-align: center; height: 100%; }
        .staff-card:hover { transform: translateY(-5px); box-shadow: 0 8px 25px rgba(0, 0, 0, 0.2); }
        .staff-photo-wrapper { position: relative; width: 100%; padding-top: 125%; overflow: hidden; border-bottom: 3px solid <?php echo $staff_color_accent; ?>; }
        .staff-photo { position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; border-radius: 10px 10px 0 0; }
        .staff-name { font-size: 1.2rem; font-weight: 600; color: <?php echo $staff_color_primary; ?>; margin-bottom: 5px; }
        .staff-position-primary { font-size: 0.9rem; color: #555; margin-bottom: 2px; }
        .staff-position-secondary { font-size: 0.8rem; color: #888; margin-bottom: 10px; }
        .btn-add-staff { background-color: <?php echo $staff_color_primary; ?>; border-color: <?php echo $staff_color_primary; ?>; color: white; transition: background-color 0.3s; }
        .btn-add-staff:hover { background-color: #A52A2A; border-color: #A52A2A; color: white; }
        .head-staff-card { width: 100%; max-width: 300px; margin: 0 auto; }
        .staff-card.head-staff-card { height: auto !important; margin-bottom: 40px !important; }
        #staffEditModal .modal-body { max-height: calc(100vh - 180px) !important; overflow-y: auto !important; overflow-x: hidden; }
        .img-preview { max-width: 100%; height: auto; max-height: 150px; object-fit: contain; border: 1px solid #ddd; padding: 5px; border-radius: 5px; }
        #deleteConfirmModal .modal-content { border: 3px solid <?php echo $staff_color_primary; ?>; }
    </style>
</head>
<body>
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                ...
                <li class="nav-item"><a class="nav-link" href="support_staff.php">ภารโรง/สนับสนุน</a></li>
...
<main class="container my-5">
    <div class="thai-staff-section">
        ...
        <h2 class="thai-staff-header display-5 mb-5"><i class="fas fa-flask me-3"></i> <?php echo $staff_group_title; ?></h2>
        ...
        <?php if (!empty($staff_list)): ?>
            ...
            <?php $other_staff = array_slice($staff_list, 1); ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color: <?php echo $staff_color_primary; ?>;">
                <h5 class="modal-title"><i class="fas fa-user-edit me-1"></i> เพิ่ม/แก้ไข บุคลากรกลุ่มสาระวิทยาศาสตร์</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="position-th" name="position_th" required>
                            ...
                            <option value="หัวหน้ากลุ่มสาระการเรียนรู้วิทยาศาสตร์และเทคโนโลยี">หัวหน้ากลุ่มสาระการเรียนรู้วิทยาศาสตร์และเทคโนโลยี</option>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-3">
                            <label for="sort-order" class="form-label">ลำดับ</label>
        ...
        <div class="modal-content border-danger">
            ...
            <form method="POST">
                ...
                <div class="modal-footer justify-content-center">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>
                    <button type="submit" class="btn btn-danger"><i class="fas fa-trash-alt me-1"></i> ลบ</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // --- 1. จัดการ Dropdown "อื่นๆ" ของตำแหน่งรอง ---
        ...
        // --- 2. จัดการ Modal แก้ไขและเพิ่มข้อมูล ---
        ...
        if (staffEditModal) {
            staffEditModal.addEventListener('show.bs.modal', function (event) {
                ...
                if (staffId > 0) {
                    ...
                    document.getElementById('sort-order').value = maxSortOrder > 0 ? (maxSortOrder + 1) : 10;
                ...
                document.getElementById('image-file').value = ''; // เคลียร์ช่องอัพโหลดไฟล์เสมอ
            });
        }
        ...
        if (deleteConfirmModal) {
            deleteConfirmModal.addEventListener('show.bs.modal', function (event) {
                const button = event.relatedTarget;
                document.getElementById('delete-staff-id').value = button.getAttribute('data-id');
                document.getElementById('staff-name-to-delete').textContent = button.getAttribute('data-name');
            });
        }
    });
</script>
