...
// =======================================================
$staff_group = 'สนับสนุน'; // ** ชื่อกลุ่มหลักใน DB **
$staff_group_title_main = 'บุคลากรฝ่ายสนับสนุน (ธุรการ/ภารโรง)'; 
$staff_group_title_header = 'บุคลากรฝ่ายสนับสนุน (ธุรการ/ภารโรง)'; // หัวข้อที่แสดงผลบนแถบสี

$staff_color_primary = '#174A6A'; // Dark Navy 
$staff_color_secondary = '#F0F8FF'; // AliceBlue (พื้นหลัง)
$staff_color_accent = '#808080'; // Gray Accent
...
// --- Logic สำหรับแบ่งกลุ่มบุคลากรเป็น 2 ส่วน (ไม่มีคนตรงกลาง) ---
$section_admin_support = []; 
$section_janitor_driver = []; 

if (!empty($staff_list)) {
    foreach ($staff_list as $staff) {
        $group_type = strtolower($staff['contact_email']); // ใช้ contact_email เป็นตัวแบ่งกลุ่ม
        
        if ($group_type === 'admin_support') {
            $section_admin_support[] = $staff;
        } else {
            // ถ้าเป็น janitor_driver หรือไม่ระบุ
            $section_janitor_driver[] = $staff;
        }
    }
}
...
if ($is_admin && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['staff_action'])) {
    ...
    // รับค่าจาก Dropdown (ประเภทบุคลากรย่อย)
    $group_type_select = $conn->real_escape_string($_POST['group_type_select'] ?? ''); 
    $email = ''; // Clear email input from modal for security
    ...
    if ($action == 'add' || $action == 'edit') {
        ...
        if (empty($error)) {
            // *** สำคัญ: contact_email ถูกใช้สำหรับเก็บ group_type_select ***
            if ($staff_id > 0) {
                $sql = "UPDATE staff_directory SET 
                        ...
                        image_file = '$image_name', contact_email = '$group_type_select', sort_order = '$sort' 
                        WHERE id = $staff_id";
                $message = "แก้ไขข้อมูลบุคลากรเรียบร้อยแล้ว";
                 ...
                 if (empty($image_name) && !isset($_FILES["image_file"])) { $error = "กรุณาเลือกไฟล์รูปภาพสำหรับการเพิ่มรายการใหม่."; }
                    ...
                    $sql = "INSERT INTO staff_directory (group_name, title_th, full_name_th, position_th, position_secondary, image_file, contact_email, sort_order) 
                            VALUES ('$staff_group', '$title', '$full_name', '$position', '$position_secondary', '$image_name', '$group_type_select', '$sort')";
...
<head>
    ...
    <title>บุคลากรฝ่ายสนับสนุน | <?php echo $staff_group_title_main; ?></title>
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                ...
                <li class="nav-item"><a class="nav-link" href="executive_staff.php">คณะผู้บริหาร</a></li>
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="navbarDropdownAcademic" role="button" data-bs-toggle="dropdown">
                        กลุ่มสาระอื่น ๆ
                    </a>
                    <ul class="dropdown-menu">
                        <?php if (isset($academic_groups_data)): ?>
                            <?php foreach ($academic_groups_data as $group): ?>
                            <li><a class="dropdown-item" href="<?php echo $group['link']; ?>"><?php echo $group['name']; ?></a></li>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </ul>
                </li>
                <li class="nav-item"><a class="nav-link active" href="support_staff.php">ภารโรง/สนับสนุน</a></li>
...
<main class="container my-5">
    <div class="thai-staff-section">
        ...
        <h2 class="thai-staff-header display-5 mb-5"><i class="fas fa-wrench me-3"></i> <?php echo $staff_group_title_header; ?></h2>
        
        <?php if ($is_admin): ?>
        <button type="button" class="btn btn-add-staff mb-4 w-100" data-bs-toggle="modal" data-bs-target="#staffEditModal" data-staff-id="0">
            <i class="fas fa-user-plus me-1"></i> เพิ่มบุคลากรใหม่ในกลุ่มฝ่ายสนับสนุน
        </button>
        <?php endif; ?>

        <?php if (!empty($staff_list)): ?>
            
            <h3 class="staff-sub-header" id="section-admin"><i class="fas fa-user-tie me-2"></i> บุคลากรฝ่ายสนับสนุนการศึกษา</h3>
            <?php if (!empty($section_admin_support)): ?>
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4 staff-grid-row"> 
                <?php foreach ($section_admin_support as $staff): ?>
                    <div class="col">
                            ...
                            <div class="staff-info d-flex flex-column h-100 p-3">
                                ...
                                <div class="mt-auto pt-2">
                                    <button type="button" class="btn btn-sm btn-warning me-1 btn-edit-staff" 
                                            ...
                                            data-group-type="<?php echo htmlspecialchars($staff['contact_email']); ?>"
            ...
            <?php else: ?>
                 <p class="text-center text-muted p-3 border rounded">ไม่มีรายการบุคลากรฝ่ายธุรการ/สนับสนุน</p>
            <?php endif; ?>
            
            
            <h3 class="staff-sub-header mt-5" id="section-janitor"><i class="fas fa-broom me-2"></i> นักการภารโรง แม่บ้าน และคนขับรถ</h3>
            <?php if (!empty($section_janitor_driver)): ?>
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4 staff-grid-row"> 
                <?php foreach ($section_janitor_driver as $staff): ?>
                    <div class="col">
                        <div class="card staff-card h-100">
                            <div class="staff-photo-wrapper">
                                <img src="<?php echo $upload_dir . htmlspecialchars($staff['image_file'] ?? 'placeholder.jpg'); ?>" 
            ...
            <?php else: ?>
                 <p class="text-center text-muted p-3 border rounded">ไม่มีรายการนักการภารโรง/แม่บ้าน</p>
            <?php endif; ?>
            
        <?php else: ?>
            <div class="col-12">
                <p class="text-center text-muted p-5 border rounded">ยังไม่มีรายการบุคลากรในฝ่ายสนับสนุน</p>
            </div>
        <?php endif; ?>
    </div> 
</main>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color: <?php echo $staff_color_primary; ?>;">
                <h5 class="modal-title" id="staffEditModalLabel"><i class="fas fa-user-edit me-1"></i> เพิ่ม/แก้ไข บุคลากรฝ่ายสนับสนุน</h5>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="group-type-select" class="form-label">ประเภทบุคลากรย่อย (ใช้แบ่งกรอบ) <span class="text-danger">*</span></label>
                        <select class="form-select" id="group-type-select" name="group_type_select" required>
                            <option value="admin_support">บุคลากรฝ่ายสนับสนุนการศึกษา/ธุรการ</option>
                            <option value="janitor_driver">นักการภารโรง/แม่บ้าน/คนขับรถ</option>
                        </select>
                    </div>
                    
                    <div class="mb-3">
                        <label for="position-th" class="form-label">ตำแหน่ง/หน้าที่หลัก <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="position-th" name="position_th" required>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-9 mb-3">
                            ...
                            <select class="form-select" id="position-secondary" name="position_secondary">
                                ...
                                <option value="หัวหน้างานธุรการ">หัวหน้างานธุรการ</option>
                                <option value="เจ้าหน้าที่ประสานงาน">เจ้าหน้าที่ประสานงาน</option>
        ...
        if (staffEditModal) {
            staffEditModal.addEventListener('show.bs.modal', function (event) {
                ...
                if (staffId > 0) {
                    ...
                    const predefinedOptions = ['', 'หัวหน้างานธุรการ', 'เจ้าหน้าที่ประสานงาน', 'ลูกจ้างประจำ', 'ลูกจ้างชั่วคราว'];
                    ...
                    // ดึงค่าประเภทบุคลากรย่อย
                    document.getElementById('group-type-select').value = button.getAttribute('data-group-type') || 'janitor_driver';
                    ...
                    // Add Mode
                    staffEditModal.querySelector('.modal-title').textContent = 'เพิ่มบุคลากรฝ่ายสนับสนุน';
                    ...
                    document.getElementById('group-type-select').value = 'admin_support'; // Default
