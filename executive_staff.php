...
// =======================================================
$staff_group = 'บริหาร'; // ** ชื่อกลุ่มใน DB **
$staff_group_title_main = 'คณะผู้บริหารและผู้ช่วยผู้บริหาร'; 
$staff_group_title_header = 'คณะผู้บริหารโรงเรียน'; // หัวข้อที่แสดงผลบนแถบสี

$staff_color_primary = '#003366'; // Dark Blue / กรมท่าเข้ม
$staff_color_secondary = '#F5F5F5'; // White Smoke (พื้นหลัง)
$staff_color_accent = '#FFD700'; // Gold (สีเน้น)
...
// --- Logic สำหรับแบ่งกลุ่มบุคลากร (ใช้ทั้งหมดเป็นคณะบริหาร) ---
$director_staff = null; 
$executive_members = [];

if (!empty($staff_list)) {
    $director_staff = $staff_list[0]; 
    $executive_members = array_slice($staff_list, 1);
}

// --- Academic Groups Data (สำหรับ Dropdown กลุ่มสาระอื่น ๆ) ---
    ...
    if ($action == 'add' || $action == 'edit') {
        ...
        if (empty($error)) {
            ...
            if (!empty($sql) && empty($error) && !$conn->query($sql)) { 
                $error = "ไม่สามารถบันทึกข้อมูลได้: " . $conn->error; $message = ''; 
            }
        }
...
<head>
    ...
    <title><?php echo $staff_group_title_header; ?> | ทำเนียบผู้บริหาร</title>
    ...
    <style>
        /* CSS เฉพาะสำหรับคณะผู้บริหาร */
        
        body {
            background-color: white !important;
        }
        
        .thai-staff-section { /* ใช้คลาสเดิมเพื่อคงคอนเซ็ปต์ */
            background-color: <?php echo $staff_color_secondary; ?>; /* White Smoke */
            padding: 40px 20px; 
            border-radius: 10px;
            box-shadow: 0 0 15px rgba(0,0,0,0.05);
        }
        
        /* สไตล์สำหรับส่วนย่อย (Section Header) */
        .staff-sub-header {
            color: <?php echo $staff_color_primary; ?>; /* Dark Blue */
            font-weight: 700;
            border-bottom: 3px solid <?php echo $staff_color_accent; ?>; /* Gold */
            padding-bottom: 5px;
            margin-top: 30px;
            margin-bottom: 20px;
        }
        ...
        .thai-staff-header {
            color: <?php echo $staff_color_primary; ?>; /* Dark Blue */
            font-weight: 700;
            border-bottom: 5px double <?php echo $staff_color_accent; ?>; /* Gold */
        ...
        .staff-card {
            border: 2px solid <?php echo $staff_color_accent; ?>; /* Gold */
            border-radius: 15px;
...
<main class="container my-5">
    <div class="thai-staff-section">
        ...
        <h2 class="thai-staff-header display-5 mb-5"><i class="fas fa-user-tie me-3"></i> <?php echo $staff_group_title_header; ?></h2>
        ...
        <button type="button" class="btn btn-add-staff mb-4 w-100" data-bs-toggle="modal" data-bs-target="#staffEditModal" data-staff-id="0" id="btn-add-staff">
            <i class="fas fa-user-plus me-1"></i> เพิ่มบุคลากรคณะผู้บริหาร
        </button>
        
        <div class="alert alert-info small text-center">
            **คำแนะนำสำหรับ Admin:** ผู้อำนวยการควรมี **ลำดับ (Sort Order) เป็น 1** เพื่อให้แสดงผลเป็นคนแรกสุด
        </div>
        ...
        <?php if (!empty($staff_list)): ?>
            ...
            <?php 
            // 1. ผู้อำนวยการ (คนแรก) - แสดงคนเดียว
            $director_staff = $staff_list[0]; 
            ?>
            <h3 class="staff-sub-header text-center" id="section-director"><i class="fas fa-crown me-2"></i> ผู้อำนวยการโรงเรียน</h3>
            <div class="row justify-content-center">
                <div class="col-12 text-center">
                    <div class="card staff-card head-staff-card">
                        <div class="staff-photo-wrapper"> 
                            <img src="<?php echo $upload_dir . htmlspecialchars($director_staff['image_file'] ?? 'placeholder.jpg'); ?>" 
                                 alt="<?php echo formatFullName($director_staff['title_th'], $director_staff['full_name_th']); ?>" 
                                 class="staff-photo">
                        </div>
                        <div class="staff-info d-flex flex-column p-3">
                            <p class="staff-name mb-1"><?php echo formatFullName($director_staff['title_th'], $director_staff['full_name_th']); ?></p>
                            <p class="staff-position-primary"><?php echo htmlspecialchars($director_staff['position_th']); ?></p>
                            <?php if (!empty($director_staff['position_secondary'])): ?>
                                <p class="staff-position-secondary"><?php echo htmlspecialchars($director_staff['position_secondary']); ?></p>
                            <?php endif; ?>

                            <?php if ($is_admin): ?>
                                <div class="mt-2">
                                    <button type="button" class="btn btn-sm btn-warning me-1 btn-edit-staff" 
                                            data-bs-toggle="modal" data-bs-target="#staffEditModal" data-id="<?php echo $director_staff['id']; ?>"
                                            data-title="<?php echo htmlspecialchars($director_staff['title_th']); ?>" data-name="<?php echo htmlspecialchars($director_staff['full_name_th']); ?>"
                                            data-position="<?php echo htmlspecialchars($director_staff['position_th']); ?>"
                                            data-position-secondary="<?php echo htmlspecialchars($director_staff['position_secondary']); ?>"
                                            data-image="<?php echo htmlspecialchars($director_staff['image_file']); ?>" data-order="<?php echo $director_staff['sort_order']; ?>">
                                        <i class="fas fa-pencil-alt"></i> แก้ไข
                                    </button>
                                    <button type="button" class="btn btn-sm btn-danger btn-delete-staff" 
                                            data-bs-toggle="modal" data-bs-target="#deleteConfirmModal"
                                            data-id="<?php echo $director_staff['id']; ?>"
                                            data-name="<?php echo formatFullName($director_staff['title_th'], $director_staff['full_name_th']); ?>">
            ...
            <?php 
            // 2. คณะบริหารที่เหลือ (Starting from the second member)
            $executive_members = array_slice($staff_list, 1); 
            ?>
            
            <?php if (!empty($executive_members)): ?>
            <h3 class="staff-sub-header text-center mt-5 mb-4" id="section-executive"><i class="fas fa-users me-2"></i> คณะบริหารและผู้ช่วย</h3>
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 justify-content-center g-4 staff-grid-row"> 
                <?php foreach ($executive_members as $staff): ?>
                    <div class="col">
        ...
        <?php else: ?>
            <div class="col-12">
                <p class="text-center text-muted p-5 border rounded">ยังไม่มีรายการบุคลากรในคณะผู้บริหาร</p>
            </div>
        <?php endif; ?>
    </div> 
</main>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color: <?php echo $staff_color_primary; ?>;">
                <h5 class="modal-title" id="staffEditModalLabel"><i class="fas fa-user-edit me-1"></i> เพิ่ม/แก้ไข บุคลากรคณะผู้บริหาร</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="position-th" name="position_th" required>
                            ...
                            <option value="ผู้ช่วยผู้อำนวยการ">ผู้ช่วยผู้อำนวยการ</option>
                    ...
                    <div class="row">
                        <div class="col-md-9 mb-3">
                            ...
                            <select class="form-select" id="position-secondary" name="position_secondary">
                                ...
                                <option value="รองผู้อำนวยการสถานศึกษา">รองผู้อำนวยการสถานศึกษา</option>
                                <option value="ผู้ช่วยผู้อำนวยการ">ผู้ช่วยผู้อำนวยการ</option>
                                <option value="หัวหน้ากลุ่มบริหารงานวิชาการ">หัวหน้ากลุ่มบริหารงานวิชาการ</option>
                                <option value="หัวหน้ากลุ่มบริหารงานบุคคล">หัวหน้ากลุ่มบริหารงานบุคคล</option>
                                <option value="หัวหน้ากลุ่มบริหารงานงบประมาณ">หัวหน้ากลุ่มบริหารงานงบประมาณ</option>
                                <option value="หัวหน้ากลุ่มบริหารงานทั่วไป">หัวหน้ากลุ่มบริหารงานทั่วไป</option>
                                <option value="ครูเชี่ยวชาญพิเศษ">ครูเชี่ยวชาญพิเศษ</option>
                                <option value="ครูเชี่ยวชาญ">ครูเชี่ยวชาญ</option>
                                <option value="ครูชำนาญการพิเศษ">ครูชำนาญการพิเศษ</option>
                                <option value="ครูชำนาญการ">ครูชำนาญการ</option>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <div class="form-text">**ผู้อำนวยการควรเป็น 1**</div>
                        </div>
                    </div>
                </div>
        ...
        if (staffEditModal) {
            staffEditModal.addEventListener('show.bs.modal', function (event) {
                ...
                if (staffId > 0) {
                    ...
                    const predefinedOptions = ['', 'รองผู้อำนวยการสถานศึกษา', 'ผู้ช่วยผู้อำนวยการ', 'หัวหน้ากลุ่มบริหารงานวิชาการ', 'หัวหน้ากลุ่มบริหารงานบุคคล', 'หัวหน้ากลุ่มบริหารงานงบประมาณ', 'หัวหน้ากลุ่มบริหารงานทั่วไป', 'ครูเชี่ยวชาญพิเศษ', 'ครูเชี่ยวชาญ', 'ครูชำนาญการพิเศษ', 'ครูชำนาญการ', 'ครู', 'ครูผู้ช่วย'];
                    ...
                    // Add Mode
                    staffEditModal.querySelector('.modal-title').textContent = 'เพิ่มบุคลากรคณะผู้บริหาร';
